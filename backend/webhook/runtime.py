...
from typing import Any, Callable, Dict, Set, List, Awaitable
...
class WebhookRuntime:
    ...
    # When set, the router will accept signatures generated by ANY of these secrets.
    meta_app_secrets: List[str]
    # Explicit override to accept unsigned webhooks (for local/dev only).
    # In production this should remain False.
    allow_unsigned_webhooks: bool
    # Optional dynamic resolver for per-workspace / per-phone webhook secrets.
    # Called by router on signature mismatch as a best-effort fallback.
    resolve_webhook_secrets: Callable[[str, dict], Awaitable[List[str]]] | None
