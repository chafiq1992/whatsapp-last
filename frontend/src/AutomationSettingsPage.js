...
import { BarChart3, Bot, MessageSquareText, Users, Settings as SettingsIcon, BookOpen, Home, UserCog, Tag, Search, Package } from 'lucide-react';
...
function OrderSearchPanel() {
  ...
  const doSearch = async (e) => {
    ...
    setSearched(false);
    try {
      const res = await api.get('/admin/orders/search', { params: { q } });
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white/70 backdrop-blur shadow-sm">
      <div className="p-6">
        <h2 className="text-lg font-semibold text-slate-800 mb-4">Order Lookup</h2>
        <p className="text-sm text-slate-500 mb-4">Search by order number to see who collected or fulfilled the order.</p>

        <form onSubmit={doSearch} className="flex items-center gap-3 mb-6">
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input
              type="text"
              className="w-full pl-10 pr-4 py-2.5 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-400/40 focus:border-orange-400"
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-slate-800 text-white text-sm font-medium hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
        ...
        {error && (
          <div className="mb-4 p-3 rounded-lg border border-rose-200 bg-rose-50 text-rose-700 text-sm">{error}</div>
        )}

        {searched && results.length === 0 && (
          <div className="text-center py-12 text-slate-400 text-sm">
            No orders found matching "<span className="font-medium text-slate-600">{query}</span>"
          </div>
        )}
        ...
        {results.length > 0 && (
          <div className="space-y-3">
            {results.map((r, i) => (
              <div key={r.shopify_order_id || i} className="border border-slate-200 rounded-xl p-4 bg-white hover:shadow-sm transition-shadow">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {statusBadge(r.financial_status)}
                      {statusBadge(r.fulfillment_status)}
                      {r.payout_status && statusBadge(r.payout_status)}
                    </div>
                    {r.customer_name && (
                      <div className="text-sm text-slate-600">
                        Customer: <span className="font-medium">{r.customer_name}</span>
                        {r.customer_phone && <span className="ml-2 text-slate-400">{r.customer_phone}</span>}
                      </div>
                    )}
                    {r.total_price && (
                      <div className="text-sm text-slate-500 mt-0.5">
                        Total: <span className="font-medium">{r.total_price} {r.currency}</span>
                      </div>
                    )}
                    {r.created_at && (
                  ...
                  <div className="text-right shrink-0">
                    {r.collected_by ? (
                      <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
                        <div className="text-xs text-emerald-600 font-medium uppercase tracking-wide">Collected by</div>
                        <div className="text-sm font-semibold text-emerald-800 mt-0.5">{r.collected_by}</div>
                        {r.collected_at && (
                          <div className="text-xs text-emerald-500 mt-0.5">{new Date(r.collected_at).toLocaleString()}</div>
                        )}
                      </div>
                    ) : (
                      <div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
                        <div className="text-xs text-slate-400">No collector info</div>
                      </div>
                    )}
                  </div>
                </div>

                {r.fulfillments && r.fulfillments.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-slate-100">
                    <div className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-1">Fulfillments</div>
                    <div className="space-y-1">
                      {r.fulfillments.map((f, fi) => (
                        <div key={fi} className="flex items-center gap-2 text-sm text-slate-600">
                          {statusBadge(f.status)}
                          {f.tracking_company && <span className="text-slate-500">{f.tracking_company}</span>}
                          {f.created_at && <span className="text-xs text-slate-400">{new Date(f.created_at).toLocaleString()}</span>}
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {r.tags && r.tags.length > 0 && (
                  <div className="mt-2 flex items-center gap-1 flex-wrap">
                    {r.tags.map((tag, ti) => (
                      <span key={ti} className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded text-xs">{tag}</span>
                    ))}
                  </div>
                )}

                {r.note && (
                  <div className="mt-2 text-xs text-slate-500 italic">Note: {r.note}</div>
                )}
              </div>
            ))}
          </div>
...
export default function AutomationSettingsPage() {
  ...
  const detectTabFromLocation = () => {
    try {
      ...
      if (key.includes('/settings/orders') || key.includes('/#/settings/orders')) return 'orders';
      ...
      <header className="h-14 px-4 flex items-center justify-between border-b bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 text-white sticky top-0 z-50">
        ...
        <div className="flex items-center justify-center flex-1">
          <div className="flex items-center gap-1 bg-white/10 border border-white/10 rounded-xl p-1">
            ...
            <button
              type="button"
              className={`px-3 py-1.5 text-sm rounded-lg ${activeTab === 'orders' ? 'bg-orange-400/20 border border-orange-300/20' : 'hover:bg-white/10'}`}
              onClick={() => goTab('orders')}
              title="Order lookup"
            >
              <span className="inline-flex items-center gap-2"><Package className="w-4 h-4" />Orders</span>
            </button>
            <button
              type="button"
